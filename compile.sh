...
if [ "$DO_STATIC" == "yes" ]; then
	...
	if [ "$HAVE_XDEBUG" == "yes" ]; then
	  write_out "warning" "Xdebug on a static build is experimental/debug-only"
	fi
fi
...
# The android-aarch64 musl-cross-make toolchain only ships a static
# libgcc.a, no libgcc_s. Xdebug's link uses -static/-Wl,-static, which
# makes the linker search for a STATIC library under this name -
# libgcc_s.a, not libgcc_s.so. (Confirmed directly: a manual replay of the
# real failing link command with -Wl,--verbose showed ld's actual search
# list only ever tried "libgcc_s.a" paths - a libgcc_s.so placed earlier
# was simply never a candidate under static linking, which is why every
# previous attempt at this appeared to place the file correctly yet still
# failed identically.)
#
# `ld --verbose` reports this toolchain's default SEARCH_DIR entries as
# absolute root-level paths (e.g. /aarch64-linux-musl/lib) hardcoded at
# the time it was built - these don't exist on a CI runner where the
# toolchain is installed under a workspace-relative directory instead of
# the real filesystem root. GNU ld identifies a linker script by its
# CONTENT, not its extension, so the same GROUP() trick works equally
# well named .a - we just had the wrong filename this whole time.
#
# Placed here, immediately before the Xdebug build (not earlier in the
# script), to avoid any theoretical risk of something between here and
# the Xdebug build touching /aarch64-linux-musl.
#
# Needs root - silently skipped if sudo isn't available (e.g. a non-CI/
# local build), since it only matters for the Xdebug build immediately
# following this block.
if [ "$COMPILE_FOR_ANDROID" == "yes" ] && [ "$HAVE_XDEBUG" == "yes" ]; then
	if type sudo >> "$DIR/install.log" 2>&1; then
		write_status "working around missing libgcc_s.a in android-aarch64 toolchain"
		ANDROID_GCC_LIBDIR="$($CC -print-libgcc-file-name | xargs dirname)"
		if [ -f "$ANDROID_GCC_LIBDIR/libgcc.a" ]; then
			ANDROID_LINKER_SCRIPT="GROUP ( $ANDROID_GCC_LIBDIR/libgcc.a )"
			# Every directory a real failing build's `ld -Wl,--verbose`
			# output was confirmed to actually search for -lgcc_s, in the
			# order it tried them, plus the toolchain-relative dir as
			# belt-and-braces.
			for ANDROID_LIBGCC_S_DIR in \
				"$INSTALL_DIR/lib" \
				"$ANDROID_GCC_LIBDIR" \
				"/aarch64-linux-musl/lib" \
				"/aarch64-linux-musl/lib64" \
				"/aarch64-linux-musl/usr/local/lib64" \
				"/aarch64-linux-musl/usr/local/lib" \
				"/aarch64-linux-musl/usr/lib64" \
				"/aarch64-linux-musl/usr/lib" \
			; do
				sudo mkdir -p "$ANDROID_LIBGCC_S_DIR" >> "$DIR/install.log" 2>&1
				if [ ! -e "$ANDROID_LIBGCC_S_DIR/libgcc_s.a" ]; then
					echo "$ANDROID_LINKER_SCRIPT" | sudo tee "$ANDROID_LIBGCC_S_DIR/libgcc_s.a" >> "$DIR/install.log" 2>&1
				fi
			done
			# Verify the file is actually readable right now, in this same
			# breath, rather than assuming it persisted correctly.
			if [ -r /aarch64-linux-musl/lib/libgcc_s.a ]; then
				write_done
			else
				write_out "WARNING" "libgcc_s.a was created but is not readable immediately afterward; Xdebug build for Android may still fail"
			fi
		else
			write_out "WARNING" "libgcc.a not found for $CC; skipping libgcc_s.a workaround, Xdebug build for Android may fail"
		fi
	else
		write_out "WARNING" "sudo not available; skipping libgcc_s.a workaround, Xdebug build for Android may fail"
	fi
fi
...
if [[ "$HAVE_XDEBUG" == "yes" ]]; then
	...
	# --host + -static-libgcc: without --host, autoconf can't tell this is a
	# cross-compile and aborts with "cannot run C compiled programs". Without
	# -static-libgcc at configure time, libtool bakes a dynamic -lgcc_s
	# reference into its generated link recipe.
	CFLAGS="$CFLAGS -static-libgcc" LDFLAGS="$LDFLAGS -static-libgcc" ./configure --with-php-config="$INSTALL_DIR/bin/php-config" $CONFIGURE_FLAGS >> "$DIR/install.log" 2>&1
	write_compile
	...
	echo "xdebug.mode=develop,profile" >> "$INSTALL_DIR/bin/php.ini" 2>&1
	...
	echo "xdebug.output_dir=/data/data/com.termux/files/home/xdebug-out" >> "$INSTALL_DIR/bin/php.ini" 2>&1
	write_done
	write_out INFO "Xdebug is included and enabled (mode=develop,profile). Adjust xdebug.mode in php.ini as needed."
fi
