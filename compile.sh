...
# there that simply pulls in the real static libgcc.a. Only relevant when
# both cross-compiling for Android and building Xdebug; harmless to skip
# otherwise. Needs root - silently skipped if sudo isn't available (e.g.
# a non-CI/local build), since it only matters for the Xdebug build later.
