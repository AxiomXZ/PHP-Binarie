...
# libgcc.a, no libgcc_s.so. Xdebug's -shared link references -lgcc_s and
# fails with "cannot find -lgcc_s" unless a real file exists under that
# name in a directory this toolchain's `ld` actually searches by default.
# `ld --verbose` on this toolchain reports its default SEARCH_DIR entries
# as absolute root-level paths (e.g. /aarch64-linux-musl/lib) hardcoded at
# the time it was built - these don't otherwise exist on a CI runner where
# the toolchain is installed under a workspace-relative directory instead
# of the real filesystem root, so this workaround creates the one real
# root-level directory ld actually looks in and places a linker script
# there that simply pulls in the real static libgcc.a.
#
# Placed here, immediately before the Xdebug build, rather than earlier in
# the script (confirmed by a real build: placing it right after the `type
# $CC` check near the top did NOT survive to when Xdebug's link actually
# runs - the file was gone or unreadable by the time it mattered, despite
# no code in this script touching /aarch64-linux-musl in between). Doing
# it right before it's needed removes that gap entirely.
#
...
if [ "$COMPILE_FOR_ANDROID" == "yes" ] && [ "$HAVE_XDEBUG" == "yes" ]; then
	if type sudo >> "$DIR/install.log" 2>&1; then
		write_status "working around missing libgcc_s.so in android-aarch64 toolchain"
		...
		if [ -f "$ANDROID_GCC_LIBDIR/libgcc.a" ]; then
			...
			sudo mkdir -p /aarch64-linux-musl/lib >> "$DIR/install.log" 2>&1
			if [ ! -e /aarch64-linux-musl/lib/libgcc_s.so ]; then
				echo "$ANDROID_LINKER_SCRIPT" | sudo tee /aarch64-linux-musl/lib/libgcc_s.so >> "$DIR/install.log" 2>&1
			fi
			# Belt-and-braces: also cover the toolchain-relative dir, in
			# case a different android-aarch64 toolchain build resolves
			# its search paths differently.
			if [ -w "$ANDROID_GCC_LIBDIR" ] && [ ! -e "$ANDROID_GCC_LIBDIR/libgcc_s.so" ]; then
				echo "$ANDROID_LINKER_SCRIPT" > "$ANDROID_GCC_LIBDIR/libgcc_s.so"
			fi
			# Verify the file is actually readable right now, in this same
			# breath, rather than assuming it persisted correctly.
			if [ -r /aarch64-linux-musl/lib/libgcc_s.so ]; then
				write_done
			else
				write_out "WARNING" "libgcc_s.so was created but is not readable immediately afterward; Xdebug build for Android may still fail"
			fi
		else
			write_out "WARNING" "libgcc.a not found for $CC; skipping libgcc_s.so workaround, Xdebug build for Android may fail"
		fi
	else
		write_out "WARNING" "sudo not available; skipping libgcc_s.so workaround, Xdebug build for Android may fail"
	fi
fi
